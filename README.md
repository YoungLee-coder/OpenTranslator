...
```
apps/web                 # Vite + React SPA（构建产物输出到 apps/api/dist，随 Worker 一起部署）
  src/routes             #   翻译页 / 登录页 / Dashboard
  src/features           #   功能模块注册表（Dashboard 动态渲染）
apps/api                 # Hono Worker（REST/SSE + 静态资源服务 → Cloudflare Workers）
  src/providers          #   供应商 adapter + 注册表 + 表单 schema
  src/routes             #   translate / auth / admin-*
  src/db                 #   D1 表结构 + 幂等初始化器
...
Dashboard → Workers & Pages → Create → Workers → Import a repository → 选仓库，Root directory 填 `/`，其余保持默认即可。

> 构建命令已写进 `apps/api/wrangler.toml` 的 `[build]` 字段，Cloudflare 会自动读取，Dashboard 里无需手填 build / deploy 命令。Cloudflare 也会自动执行 `pnpm install --frozen-lockfile` 安装依赖。
...
pnpm --filter @opentranslator/web build       # 构建前端到 apps/api/dist
