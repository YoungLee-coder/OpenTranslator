...
6. **DB 迁移安全**：若改了 `src/db/schema.sql`，确认本地 `pnpm db:init` 验证过；部署后需 `POST /api/init`（头 `X-Init-Secret`）幂等建表——确认这点已告知。
7. **敏感端点**：确认 `POST /api/init` 的 secret 没出现在 URL / 文档 / 提交信息 / 分享的截图。
