export const zhCN = {
  "common.loading": "加载中…",
  "common.saving": "保存中…",
  "common.save": "保存",
  "common.saveSettings": "保存设置",
  "common.saveChanges": "保存修改",
  "common.cancel": "取消",
  "common.retry": "重试",
  "common.default": "默认",
  "common.copy": "复制",
  "common.copied": "已复制",
  "common.stop": "停止",
  "common.chars": "{{count}} 字符",
  "common.enabled": "已启用",
  "common.disabled": "已停用",
  "common.operationFailed": "操作失败",
  "common.close": "关闭",
  "common.delete": "删除",
  "common.edit": "编辑",
  "common.add": "新增",
  "common.processing": "处理中…",
  "common.submitting": "提交中…",
  "common.noChanges": "没有变更",
  "common.defaultSuffix": " · 默认",

  "nav.home": "OpenTranslator 首页",
  "nav.translate": "翻译",
  "nav.write": "写作",
  "nav.dashboard": "控制台",
  "nav.login": "登录",
  "nav.menu": "菜单",
  "nav.more": "更多",

  "theme.toggle": "切换主题",
  "theme.light": "浅色",
  "theme.dark": "深色",

  "locale.label": "语言",
  "locale.zhCN": "中文",
  "locale.en": "English",

  "user.menu": "用户菜单",
  "user.logout": "退出登录",

  "auth.loginTitle": "登录控制台",
  "auth.email": "邮箱",
  "auth.username": "用户名",
  "auth.usernameHint": "可用邮箱或任意用户名",
  "auth.password": "密码",
  "auth.passwordMin": "（至少 8 位）",
  "auth.login": "登录",
  "auth.accountDisabled": "账号已被停用",

  "translator.title": "AI 翻译",
  "translator.swapLanguages": "交换语言",
  "translator.translate": "翻译",
  "translator.noModel": "暂无可用模型",
  "translator.inputPlaceholder": "输入要翻译的文本…",
  "translator.translating": "翻译中…",
  "translator.translatingChunk": "翻译中…（{{current}}/{{total}}）",
  "translator.outputPlaceholder": "译文",
  "translator.copyTranslation": "复制译文",
  "translator.tooLong": "文本超过 {{max}} 字符上限",
  "translator.general": "通用",
  "translator.expert": "专家",

  "write.title": "AI 写作",
  "write.modeImprove": "润色",
  "write.modeStyle": "风格",
  "write.modeFormality": "正式度",
  "write.modeShorten": "精简",
  "write.styleSimple": "简洁",
  "write.styleBusiness": "商务",
  "write.styleAcademic": "学术",
  "write.styleCasual": "口语",
  "write.formalityFormal": "更正式",
  "write.formalityInformal": "更口语",
  "write.shortenLevelLabel": "程度",
  "write.shortenLight": "轻度",
  "write.shortenModerate": "适中",
  "write.shortenAggressive": "大幅",
  "write.styleLabel": "风格",
  "write.toneLabel": "语气",
  "write.modeAria": "写作模式",
  "write.improve": "改进",
  "write.inputPlaceholder": "输入要改进的文本…",
  "write.improving": "改进中…",
  "write.outputPlaceholder": "改进结果",
  "write.replaceSource": "替换原文",
  "write.replaceSourceTip": "用改进结果替换左侧原文",
  "write.copyResult": "复制改进结果",

  "languages.auto": "自动检测",
  "languages.zhCN": "简体中文",
  "languages.zhTW": "繁體中文（台灣）",
  "languages.zhHK": "繁體中文（香港）",

  "setup.title": "初始化 OpenTranslator",
  "setup.description":
    "部署后首次打开本页。绑定就绪后，在此建表并设定管理员账号，完成后进入控制台。",
  "setup.bindingsHint":
    "如果你是直接部署后打开本页，多半还没有自己的 D1 数据库和 KV 命名空间。请先在 Cloudflare 创建，再到本 Worker 面板里绑定。",
  "setup.stepBindingsTitle": "存储绑定",
  "setup.stepDbTitle": "数据库升级",
  "setup.stepAdminTitle": "管理员账号",
  "setup.dbBinding": "D1 数据库（binding 名 DB）",
  "setup.kvBinding": "KV 命名空间（binding 名 KV）",
  "setup.notConnected": "未连接",
  "setup.connected": "已连接",
  "setup.step1":
    "在 Cloudflare Dashboard 创建自己的 D1 数据库和 KV 命名空间（直接部署不会自动生成）",
  "setup.step2": "打开本 Worker → Settings → Bindings",
  "setup.step3": "添加 D1（binding 名填 DB）和 KV（binding 名填 KV），保存后重新检测",
  "setup.initHint":
    "首次建表需要 Worker 的 JWT_SECRET（Settings → Variables and Secrets）。密钥只用于本次请求，不会写入地址栏。",
  "setup.migrateHint":
    "检测到数据库有待执行的迁移。先完成升级，再设定管理员账号。",
  "setup.initSecret": "初始化密钥（JWT_SECRET）",
  "setup.migrateAction": "执行数据库升级",
  "setup.migrateRunning": "升级中…",
  "setup.adminHint": "创建第一个管理员账号。完成后将自动登录并进入控制台。",
  "setup.adminEmail": "管理员邮箱",
  "setup.adminUsername": "管理员用户名",
  "setup.adminPassword": "密码",
  "setup.adminPasswordConfirm": "确认密码",
  "setup.passwordMismatch": "两次输入的密码不一致",
  "setup.badSecret": "初始化密钥不正确，请核对 JWT_SECRET。",
  "setup.adminReady": "管理员已创建。",
  "setup.completeAction": "完成初始化",
  "setup.createAdminAction": "创建管理员并进入控制台",
  "setup.completeRunning": "正在初始化…",
  "setup.networkError": "无法连接 Worker",
  "setup.networkErrorHint": "请检查网络或 Worker 是否已部署，然后重新检测。",
  "setup.allReady": "初始化完成，正在进入…",
  "setup.recheck": "重新检测",

  "dashboard.title": "控制台",
  "dashboard.tabOverview": "概览",
  "dashboard.tabProviders": "供应商",
  "dashboard.tabSettings": "设置",

  "overview.title": "用量概览",
  "overview.totalRequests": "总请求数",
  "overview.totalChars": "总字符数",
  "overview.provider": "供应商",
  "overview.requests": "请求数",
  "overview.chars": "字符数",

  "profile.title": "用户信息",
  "profile.emailRequired": "邮箱不能为空",
  "profile.usernameRequired": "用户名不能为空",
  "profile.passwordRequired": "请输入当前密码以确认修改",
  "profile.passwordMin": "新密码至少 8 位",
  "profile.noChanges": "没有需要保存的修改",
  "profile.updated": "用户信息已更新",
  "profile.avatarUpdated": "头像已更新",
  "profile.avatarRemoved": "头像已移除",
  "profile.uploadAvatar": "上传头像",
  "profile.removeAvatar": "移除头像",
  "profile.avatarHint": "JPEG / PNG / WebP / GIF，最大 512 KB",
  "profile.currentPassword": "当前密码",
  "profile.newPassword": "新密码（可选）",
  "profile.passwordPlaceholder": "留空则不修改",

  "settings.title": "站点设置",
  "settings.saved": "设置已保存",
  "settings.authedRateLimit": "登录用户限流（次/分钟）",
  "settings.authedRateLimitDesc": "登录管理员每分钟最大请求数。",
  "settings.translationCache": "翻译结果缓存",
  "settings.translationCacheDesc":
    "相同文本+语言对命中 KV 缓存时直接返回，省时省额度。",
  "settings.cacheTtl": "缓存保留时长（小时）",
  "settings.cacheTtlDesc":
    "相同结果在 KV 中的存活时间，到期自动清除。范围 {{min}}–{{max}} 小时；关闭缓存时此值不生效。",
  "settings.organizeFormat": "整理格式",
  "settings.organizeFormatDesc":
    "翻译时根据杂乱原文推断段落与列表结构，输出整洁译文。仅对通用翻译生效；AI 专家与 DeepL 不受影响。",
  "settings.disableModelReasoning": "关闭模型推理",
  "settings.disableModelReasoningDesc":
    "开启后在请求侧关闭思考链 / 推理（如 reasoning、thinking），降低延迟与用量。DeepL 不受影响；部分模型可能不支持此参数。",

  "backup.title": "导入 / 导出数据",
  "backup.description":
    "仅管理员。导出站点设置、供应商（含 API Key 明文）与功能模块开关，便于备份或迁到新实例。文件含密钥，请妥善保管。不含管理员账号、头像与用量记录。导入会替换当前全部供应商并覆盖对应设置，不可撤销。",
  "backup.export": "导出",
  "backup.exporting": "导出中…",
  "backup.exported": "备份已下载",
  "backup.import": "导入",
  "backup.importing": "导入中…",
  "backup.importTitle": "导入备份",
  "backup.importDesc":
    "将导入 {{providers}} 个供应商、{{settings}} 项设置、{{modules}} 个功能模块。现有供应商会被全部替换。确认继续？",
  "backup.confirmImport": "确认导入",
  "backup.imported":
    "已导入 {{providers}} 个供应商、{{settings}} 项设置、{{modules}} 个功能模块",
  "backup.importedWithSkip":
    "已导入 {{providers}} 个供应商、{{settings}} 项设置、{{modules}} 个功能模块（跳过 {{skipped}} 项本实例不支持的数据）",
  "backup.invalidFile": "不是有效的 OpenTranslator 备份文件",
  "backup.fileTooLarge": "文件过大（上限 1 MB）",

  "modules.title": "功能模块",
  "modules.module": "模块",
  "modules.descriptionCol": "说明",
  "modules.status": "状态",
  "modules.enabledToast": "已启用「{{name}}」",
  "modules.disabledToast": "已停用「{{name}}」",

  "providers.title": "供应商",
  "providers.fieldRequired": "「{{label}}」为必填项",
  "providers.baseUrlInvalid": "Base URL 需填写 SDK 根地址（以 http:// 或 https:// 开头，勿含 /chat/completions 等路径）",
  "providers.nameKeyRequired": "显示名称和 API Key 为必填项",
  "providers.updated": "供应商已更新",
  "providers.added": "供应商已添加",
  "providers.deleted": "已删除「{{name}}」",
  "providers.enabledToast": "已启用「{{name}}」",
  "providers.disabledToast": "已停用「{{name}}」",
  "providers.empty": "还没有供应商，添加一个即可开始翻译。",
  "providers.addProvider": "新增供应商",
  "providers.name": "名称",
  "providers.type": "类型",
  "providers.models": "模型",
  "providers.status": "状态",
  "providers.actions": "操作",
  "providers.order": "顺序",
  "providers.dragHandle": "拖动以调整「{{name}}」的顺序",
  "providers.reordered": "供应商顺序已更新",
  "providers.toggleEnabled": "切换{{name}}启用状态",
  "providers.setDefault": "设为默认",
  "providers.defaultModel": "站点默认模型",
  "providers.defaultModelPick": "选择默认模型",
  "providers.defaultModelNone": "请先启用至少一个带有模型的供应商。",
  "providers.defaultModelSaved": "站点默认模型已更新",
  "providers.editTitle": "编辑供应商",
  "providers.addTitle": "新增供应商",
  "providers.formDesc": "配置模型供应商的连接信息。API Key 加密存储，明文不入库。",
  "providers.displayName": "显示名称",
  "providers.apiKey": "API Key",
  "providers.apiKeyOptional": "（留空则不修改）",
  "providers.locked": "已锁定",
  "providers.modelsHint": "一行一个，首项为默认",
  "providers.openrouterLockHint":
    "OpenRouter 可用「模型名:供应商1,供应商2」锁定上游供应商（按顺序优先），如 anthropic/claude-sonnet-4.5:anthropic,google",
  "providers.label.custom": "自定义",
  "providers.customHint": "同一把 API Key 可添加多条地址，分别对接 OpenAI / Claude / Gemini 协议。翻译时按模型自动走对应地址。",
  "providers.addEndpoint": "添加 API 地址",
  "providers.removeEndpoint": "删除",
  "providers.endpointFormat": "API 格式",
  "providers.endpointN": "地址 {{n}}",
  "providers.endpointsEmpty": "请至少添加一条 API 地址",
  "providers.endpointNeedUrl": "地址 {{n}} 的 Base URL 需以 http:// 或 https:// 开头",
  "providers.endpointNeedModel": "地址 {{n}} 请至少填写一个模型",
  "providers.endpointDuplicate": "同一模型不能出现在多条地址中：{{models}}",
  "providers.format.openai": "OpenAI 兼容",
  "providers.format.claude": "Claude",
  "providers.format.gemini": "Gemini",
  "providers.enable": "启用",
  "providers.deleteTitle": "删除供应商",
  "providers.deleteConfirm": "确认删除「{{name}}」？此操作不可撤销。",
  "providers.testLatency": "测试延迟",
  "providers.testLatencyTesting": "测试中…",
  "providers.testLatencyOk": "{{ms}} ms · {{preview}}",
  "providers.testLatencyFail": "请求失败：{{error}}",
  "providers.testLatencyNeedKey": "请先填写 API Key（编辑时可沿用已保存的密钥）",
  "providers.testLatencyNeedModel": "请先填写模型",
  "providers.testLatencyBadConfig": "配置不完整或 Base URL 无效",
  "providers.testLatencyTimeout": "请求超时",
  "providers.testLatencyUnreachable": "上游无响应或网络错误",

  "dbVersion.title": "数据库版本",
  "dbVersion.description":
    "检测 D1 schema 迁移状态，按需执行未应用的迁移。KV 缓存会在迁移后自动失效。",
  "dbVersion.pendingFound": "发现待更新迁移{{detail}}",
  "dbVersion.upToDate": "数据库已是最新版本",
  "dbVersion.migrated": "已执行 {{count}} 项迁移：{{list}}",
  "dbVersion.noMigrationNeeded": "数据库已是最新，无需迁移",
  "dbVersion.current": "当前版本",
  "dbVersion.latest": "最新版本",
  "dbVersion.uninitialized": "未初始化",
  "dbVersion.pendingUpdate": "有待更新",
  "dbVersion.latestBadge": "已是最新",
  "dbVersion.checkUpdate": "检测更新",
  "dbVersion.runUpdate": "执行更新",
  "dbVersion.migrateTitle": "执行数据库迁移",
  "dbVersion.migrateDesc":
    "将对 D1 执行未应用的 schema 迁移并清空 KV 设置缓存，操作幂等、可重复执行。确认继续？",
  "dbVersion.migrating": "迁移中…",
  "dbVersion.confirmUpdate": "确认更新",

  "dbAudit.title": "数据库一致性审计",
  "dbAudit.description":
    "扫描公开模型引用、供应商标记、AI 专家配置与术语库迁移残留等一致性问题，可按需修复。损坏的 JSON 字段需到对应管理页重新填写。",
  "dbAudit.passed": "一致性检查通过，未发现问题",
  "dbAudit.issuesFound": "发现 {{count}} 项一致性问题",
  "dbAudit.repaired": "已修复 {{count}} 项：{{list}}",
  "dbAudit.nothingToRepair": "无可修复项",
  "dbAudit.repairedOne": "已修复：{{list}}",
  "dbAudit.cannotRepair": "该项无需修复或未能修复",
  "dbAudit.noIssues": "无问题",
  "dbAudit.allPassed": "所有检查项均通过",
  "dbAudit.error": "错误",
  "dbAudit.warning": "警告",
  "dbAudit.repairing": "修复中…",
  "dbAudit.repair": "修复",
  "dbAudit.recheck": "重新检测",
  "dbAudit.repairAll": "修复全部",
  "dbAudit.repairTitle": "修复一致性问题",
  "dbAudit.repairDesc":
    "将对失效的站点/公开模型引用、越界默认模型、重复公开默认供应商标记、无效 AI 专家配置与术语库残留等执行安全修复，操作幂等、可重复执行。损坏的 JSON 字段不会自动修改，需到对应管理页重填。确认继续？",
  "dbAudit.confirmRepair": "确认修复",

  "experts.title": "AI 专家",
  "experts.description":
    "基于 沉浸式翻译 AI 专家 的专业翻译策略。启用后，用户可在翻译页选择不同专家以优化特定场景（科技、金融、GitHub 等）。",
  "experts.defaultExpert": "站点默认专家",
  "experts.generalDefault": "通用（默认提示词）",
  "experts.saved": "AI 专家配置已保存",
  "experts.saveConfig": "保存配置",
  "experts.enableExpert": "启用 {{name}}",

  "publicAccess.title": "公开访问",
  "publicAccess.description":
    "勾选对匿名访客开放的模型，并指定一个公开默认模型。与站点默认模型相互独立。",
  "publicAccess.saveFailed": "保存失败",
  "publicAccess.rateLimitSaved": "限流设置已保存",
  "publicAccess.publicModels": "公开模型",
  "publicAccess.noModels":
    "暂无已启用供应商的模型，请先在「供应商」中配置模型。",
  "publicAccess.provider": "供应商",
  "publicAccess.model": "模型",
  "publicAccess.open": "开放",
  "publicAccess.publicDefault": "公开默认",
  "publicAccess.openModel": "开放 {{model}}",
  "publicAccess.hint":
    "勾选与设默认即时保存。未开放任何模型时，匿名访客将无法翻译。",
  "publicAccess.anonRateLimit": "匿名访客限流（次/分钟）",
  "publicAccess.anonRateLimitDesc": "匿名访客每分钟最大请求数；超出会触发限流。",

  "users.title": "用户管理",
  "users.description":
    "全站仅一名管理员。可创建普通用户、分配权限、查看各账号用量，并启用或停用账号。关闭本模块后已有账号仍可登录。",
  "users.username": "用户名",
  "users.email": "邮箱",
  "users.role": "角色",
  "users.status": "状态",
  "users.permissions": "权限",
  "users.usage": "用量",
  "users.usageTitle": "使用情况",
  "users.usageDesc": "「{{name}}」的翻译与写作用量。",
  "users.usageEmpty": "该账号还没有用量记录。",
  "users.actions": "操作",
  "users.add": "新增用户",
  "users.addTitle": "新增普通用户",
  "users.addDesc": "新账号默认为普通用户。管理员可在此分配权限。",
  "users.password": "密码",
  "users.passwordMin": "密码至少 8 位",
  "users.created": "用户已创建",
  "users.empty": "还没有用户。",
  "users.roleAdmin": "管理员",
  "users.roleUser": "普通用户",
  "users.deleteTitle": "删除用户",
  "users.deleteDesc": "确定删除「{{name}}」？该账号将立即无法登录。",
  "users.deleted": "已删除「{{name}}」",
  "users.cannotDeleteSelf": "不能删除当前登录的账号",
  "users.cannotDeleteAdmin": "不能删除管理员账号",
  "users.cannotModifyAdmin": "不能修改管理员账号",
  "users.resetPassword": "重置密码",
  "users.resetTitle": "重置密码",
  "users.resetDesc": "为「{{name}}」设置新密码。对方下次登录需使用新密码。",
  "users.resetDone": "已重置「{{name}}」的密码",
  "users.usernameInvalid": "用户名为 2–64 个字符，且不能包含空格",
  "users.usernameTaken": "该用户名已被使用",
  "users.emailInvalid": "请输入有效邮箱",
  "users.emailTaken": "该邮箱已被注册",
  "users.limitReached": "已达到用户数量上限",
  "users.useProfilePassword": "请在用户信息中修改自己的账号",
  "users.featureDisabled": "多用户管理模块未启用",
  "users.editPermissions": "权限",
  "users.editPermissionsDesc": "为「{{name}}」选择可使用的功能。",
  "users.permissionsSaved": "权限已保存",
  "users.allPermissions": "全部权限",
  "users.noPermissions": "无",
  "users.toggleEnabled": "切换 {{name}} 启用状态",
  "users.enabledToast": "已启用「{{name}}」",
  "users.disabledToast": "已停用「{{name}}」",
  "users.perm.translate": "翻译",
  "users.perm.write": "写作",
  "users.perm.providers": "供应商",
  "users.perm.settings": "站点设置",
  "users.perm.usage": "用量概览",
} as const;

export type MessageKey = keyof typeof zhCN;
export type Messages = Record<MessageKey, string>;
