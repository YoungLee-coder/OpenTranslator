...
- `JWT_SECRET`：JWT 签名密钥，**同时**是 `POST /api/init`（`X-Init-Secret` 头）的访问凭证。32 位以上随机字符串。
...
- `POST /api/init` 是建表 + 种子数据的引导端点，靠请求头 `X-Init-Secret: <JWT_SECRET>` 守卫（密钥**绝不**放进 URL path）。**绝不**把真实 `JWT_SECRET` 写进文档、提交信息、分享的 URL 截图。本地调试用 `.dev.vars` 里的值。
